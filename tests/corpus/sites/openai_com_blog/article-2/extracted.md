...
We’re also building tools to help detect misleading content such as a detection classifier that can tell when a video was generated by Sora. We plan to include [C2PA metadata⁠](https://help.openai.com/en/articles/8912793-c2pa-in-dall-e-3) in the future if we deploy the model in an OpenAI product.

In addition to us developing new techniques to prepare for deployment, we’re leveraging the [existing safety methods⁠](https://cdn.openai.com/papers/DALL_E_3_System_Card.pdf) that we built for our products that use DALL·E 3, which are applicable to Sora as well.
