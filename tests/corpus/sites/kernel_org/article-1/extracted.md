...
* [Chinese (Simplified)](../translations/zh_CN/process/coding-style.html)
* [Chinese (Traditional)](../translations/zh_TW/process/coding-style.html)
* [Italian](../translations/it_IT/process/coding-style.html)
* [Spanish](../translations/sp_SP/process/coding-style.html)

# Linux kernel coding style[¶](#linux-kernel-coding-style "Permalink to this heading")
...
## 1) Indentation[¶](#indentation "Permalink to this heading")
...
## 2) Breaking long lines and strings[¶](#breaking-long-lines-and-strings "Permalink to this heading")
...
## 3) Placing Braces and Spaces[¶](#placing-braces-and-spaces "Permalink to this heading")
...
### 3.1) Spaces[¶](#spaces "Permalink to this heading")
...
## 4) Naming[¶](#naming "Permalink to this heading")
...
## 5) Typedefs[¶](#typedefs "Permalink to this heading")
...
## 6) Functions[¶](#functions "Permalink to this heading")
...
### 6.1) Function prototypes[¶](#function-prototypes "Permalink to this heading")
...
\_\_init void \* \_\_must\_check action(enum magic value, size\_t size, u8 count,
                                  char \*fmt, ...) \_\_printf(4, 5) \_\_malloc;
...
static \_\_always\_inline \_\_init \_\_printf(4, 5) void \* \_\_must\_check action(enum magic value,
               size\_t size, u8 count, char \*fmt, ...) \_\_malloc
{
       ...
}

## 7) Centralized exiting of functions[¶](#centralized-exiting-of-functions "Permalink to this heading")
...
## 8) Commenting[¶](#commenting "Permalink to this heading")
...
When commenting the kernel API functions, please use the kernel-doc format. See the files at [Documentation/doc-guide/](../doc-guide/index.html#doc-guide) and `tools/docs/kernel-doc` for details. Note that the danger of over-commenting applies to kernel-doc comments all the same. Do not add boilerplate kernel-doc which simply reiterates what’s obvious from the signature of the function.
...
## 9) You’ve made a mess of it[¶](#you-ve-made-a-mess-of-it "Permalink to this heading")
...
Note that you can also use the `clang-format` tool to help you with these rules, to quickly re-format parts of your code automatically, and to review full files in order to spot coding style mistakes, typos and possible improvements. It is also handy for sorting `#includes`, for aligning variables/macros, for reflowing text and other similar tasks. See the file [Documentation/dev-tools/clang-format.rst](../dev-tools/clang-format.html#clangformat) for more details.

Some basic editor settings, such as indentation and line endings, will be set automatically if you are using an editor that is compatible with EditorConfig. See the official EditorConfig website for more information: [https://editorconfig.org/](https://editorconfig.org/)

## 10) Kconfig configuration files[¶](#kconfig-configuration-files "Permalink to this heading")
...
For full documentation on the configuration files, see the file [Kconfig Language](../kbuild/kconfig-language.html).

## 11) Data structures[¶](#data-structures "Permalink to this heading")
...
## 12) Macros, Enums and RTL[¶](#macros-enums-and-rtl "Permalink to this heading")
...
## 13) Printing kernel messages[¶](#printing-kernel-messages "Permalink to this heading")
...
There are a number of driver model diagnostic macros in <linux/dev\_printk.h> which you should use to make sure messages are matched to the right device and driver, and are tagged with the right level: `dev_err()`, `dev_warn()`, `dev_info()`, and so forth. For messages that aren’t associated with a particular device, <linux/printk.h> defines [`pr_notice()`](../core-api/printk-basics.html#c.pr_notice "pr_notice"), [`pr_info()`](../core-api/printk-basics.html#c.pr_info "pr_info"), [`pr_warn()`](../core-api/printk-basics.html#c.pr_warn "pr_warn"), [`pr_err()`](../core-api/printk-basics.html#c.pr_err "pr_err"), etc. When drivers are working properly they are quiet, so prefer to use dev\_dbg/pr\_debug unless something is wrong.

Coming up with good debugging messages can be quite a challenge; and once you have them, they can be a huge help for remote troubleshooting. However debug message printing is handled differently than printing other non-debug messages. While the other `pr_XXX()` functions print unconditionally, [`pr_debug()`](../core-api/printk-basics.html#c.pr_debug "pr_debug") does not; it is compiled out by default, unless either DEBUG is defined or CONFIG\_DYNAMIC\_DEBUG is set. That is true for `dev_dbg()` also, and a related convention uses VERBOSE\_DEBUG to add `dev_vdbg()` messages to the ones already enabled by DEBUG.
...
## 14) Allocating memory[¶](#allocating-memory "Permalink to this heading")

The kernel provides the following general purpose memory allocators: [`kmalloc()`](../core-api/mm-api.html#c.kmalloc "kmalloc"), [`kzalloc()`](../core-api/mm-api.html#c.kzalloc "kzalloc"), [`kmalloc_array()`](../core-api/mm-api.html#c.kmalloc_array "kmalloc_array"), [`kcalloc()`](../core-api/mm-api.html#c.kcalloc "kcalloc"), [`vmalloc()`](../core-api/mm-api.html#c.vmalloc "vmalloc"), and [`vzalloc()`](../core-api/mm-api.html#c.vzalloc "vzalloc"). Please refer to the API documentation for further information about them. [Documentation/core-api/memory-allocation.rst](../core-api/memory-allocation.html#memory-allocation)
...
These generic allocation functions all emit a stack dump on failure when used without \_\_GFP\_NOWARN so there is no use in emitting an additional failure message when NULL is returned.

## 15) The inline disease[¶](#the-inline-disease "Permalink to this heading")
...
A reasonable rule of thumb is to not put inline at functions that have more than 3 lines of code in them. An exception to this rule are the cases where a parameter is known to be a compile time constant, and as a result of this constantness you _know_ the compiler will be able to optimize most of your function away at compile time. For a good example of this later case, see the [`kmalloc()`](../core-api/mm-api.html#c.kmalloc "kmalloc") inline function.
...
## 16) Function return values and names[¶](#function-return-values-and-names "Permalink to this heading")
...
For example, `add work` is a command, and the `add_work()` function returns 0 for success or -EBUSY for failure. In the same way, `PCI device present` is a predicate, and the [`pci_dev_present()`](../driver-api/pci/pci.html#c.pci_dev_present "pci_dev_present") function returns 1 if it succeeds in finding a matching device or 0 if it doesn’t.
...
## 17) Using bool[¶](#using-bool "Permalink to this heading")
...
## 18) Don’t re-invent the kernel macros[¶](#don-t-re-invent-the-kernel-macros "Permalink to this heading")

There are many header files in include/linux/ that contain a number of macros that you should use, rather than explicitly coding some variant of them yourself. For example, if you need to calculate the length of an array, take advantage of the macro
...
There are also `min()` and `max()` macros defined in minmax.h that do strict type checking if you need them. Feel free to peruse the header files to see what else is already defined that you shouldn’t reproduce in your code.

## 19) Editor modelines and other cruft[¶](#editor-modelines-and-other-cruft "Permalink to this heading")
...
## 20) Inline assembly[¶](#inline-assembly "Permalink to this heading")
...
## 21) Conditional Compilation[¶](#conditional-compilation "Permalink to this heading")
...
## 22) Do not crash the kernel[¶](#do-not-crash-the-kernel "Permalink to this heading")

In general, the decision to crash the kernel belongs to the user, rather than to the kernel developer.

### Avoid panic()[¶](#avoid-panic "Permalink to this heading")
...
### Use WARN() rather than BUG()[¶](#use-warn-rather-than-bug "Permalink to this heading")
...
### Use WARN\_ON\_ONCE() rather than WARN() or WARN\_ON()[¶](#use-warn-on-once-rather-than-warn-or-warn-on "Permalink to this heading")
...
### Do not WARN lightly[¶](#do-not-warn-lightly "Permalink to this heading")
...
### Do not worry about panic\_on\_warn users[¶](#do-not-worry-about-panic-on-warn-users "Permalink to this heading")

A few more words about panic\_on\_warn: Remember that `panic_on_warn` is an available kernel option, and that many users set this option. This is why there is a “Do not WARN lightly” writeup, above. However, the existence of panic\_on\_warn users is not a valid reason to avoid the judicious use WARN\*(). That is because, whoever enables panic\_on\_warn has explicitly asked the kernel to crash if a WARN\*() fires, and such users must be prepared to deal with the consequences of a system that is somewhat more likely to crash.

### Use BUILD\_BUG\_ON() for compile-time assertions[¶](#use-build-bug-on-for-compile-time-assertions "Permalink to this heading")
...
## Appendix I) References[¶](#appendix-i-references "Permalink to this heading")
...
Kernel CodingStyle, by [greg@kroah.com](mailto:greg%40kroah.com) at OLS 2002: [http://www.kroah.com/linux/talks/ols\_2002\_kernel\_codingstyle\_talk/html/](http://www.kroah.com/linux/talks/ols_2002_kernel_codingstyle_talk/html/)
