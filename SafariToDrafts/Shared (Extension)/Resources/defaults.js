...
(function () {
  ...
  const BASE_TEXT_CLEANUP_RULES = [
    ...
    String.raw`replace:/\[\s*advertiser content from\s+this is the title for the native ad\s*\]\([^)]*\)/gi =>`,
    ...
    String.raw`line:/^\s*(?:\[[^\]]{2,80}\]\([^)]*\)|[A-Z][A-Za-z .'-]{2,80})\s+Senior [^\n]{2,80}Reporter\s*$/`,
    String.raw`line:/^\s*(?:\[\s*)?\d{1,5}\s+comments?(?:\s*\]\([^)]*\))?\s*$/i`,
    String.raw`tail:/\n+\s*topics:\s*(?:\n+\s*(?:[-*]\s*)?[^\n]{1,80}\/?\s*){1,20}\s*$/i`,
    String.raw`line:/^\s*loading\.\.\.\s*$/i`,
    String.raw`line:/^\s*loading\s*$/i`,
    String.raw`line:/^\s*(?:\[\s*)?share(?:\s*\]\([^)]*\))?\s*$/i`,
    String.raw`tail:/\n+Share\s+\u2014[\s\S]*$/i`,
    String.raw`line:/^\s*(?:\[\s*)?share to (?:facebook|x)(?:\s*\]\([^)]*\))?\s*$/i`,
    ...
    String.raw`line:/^\s*(?:\[\s*)?join the conversation(?:\s*\]\([^)]*\))?\s*$/i`,
    ...
    String.raw`line:/^\s*(?:\[\s*)?subscribe(?:\s*\]\([^)]*\))?\s*$/i`,
    ...
    String.raw`line:/^\s*---+\s*$/`,
    String.raw`line:/^\s*(?:\[\s*)?see all topics(?:\s*\]\([^)]*\))?\s*$/i`,
    String.raw`line:/^\s*(?:\[\s*)?(?:facebook|tweet|email|link|threads|link copied!|follow)(?:\s*\]\([^)]*\))?\s*$/i`,
  ...
  function migrateEditableDefaultList(savedList, defaultList, savedRevision, options = {}) {
    ...
    if (savedRevision < DEFAULTS_REVISION && editableListLooksDefaultDerived(savedList, defaultList)) {
      if (options.preserveCustomPriority) {
        ...
        return unique([...savedCustomEntries, ...defaultList]);
      }
